...
type YunnanCrawler struct{}

func NewYunnanCrawler() *YunnanCrawler {
	return &YunnanCrawler{}
}

func (b *YunnanCrawler) FetchData() (models.Data, error) {
	lxrod := browser.Lxrod{}
	...
	rod.Try(func() {
		...
		p.MustNavigate(browser.YunnanInfoUrl)
		p.MustWaitStable()
		...
		for i, element := range table.MustElements("li") {
			...
			msg = append(msg, models.MsgSt{
				...
				Url:   browser.YunnanBaseUrl + *element.MustElement("a").MustAttribute("href"),
			})
		}
	})
	data := models.Data{
