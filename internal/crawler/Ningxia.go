...
type NingxiaCrawler struct{}

func NewNingxiaCrawler() *NingxiaCrawler {
	return &NingxiaCrawler{}
}

func (b *NingxiaCrawler) FetchData() (models.Data, error) {
	lxrod := browser.Lxrod{}
	...
	rod.Try(func() {
		...
		p.MustNavigate(browser.NingxiaInfoUrl)
		p.MustWaitStable()
		...
		for i, element := range table.MustElements("li") {
			...
			msg = append(msg, models.MsgSt{
				...
				Url:   browser.NingxiaBaseUrl + *element.MustElement("a").MustAttribute("href"),
			})
		}
	})
	data := models.Data{
